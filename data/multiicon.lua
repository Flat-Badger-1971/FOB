...
function MultiIconTimer:New()
    ...
    timer.timeline = _G.ANIMATION_MANAGER:CreateTimelineFromVirtual("MultiIconAnimation")
...
function MultiIconTimer:SetupMultiIconTexture(multiIcon)
    ...
    if multiIcon.iconData[index].iconTint then
        ...
        multiIcon:SetColor(_G.ZO_WHITE:UnpackRGBA())
    end
end
...
function FOB.MultiIconAnimation_SetAlpha(animation, alpha)
    animation:GetTimeline().object:SetAlphas(alpha)
end

function FOB.MultiIconAnimation_OnStop(timeline)
...
do
    ...
    function FOB.MultiIconOnShow(self)
        if self.iconData then
            if #self.iconData > 1 then
                ...
                if self.iconData[1].iconTint then
                    ...
                    self:SetColor(_G.ZO_WHITE:UnpackRGBA())
                end
    ...
    function FOB.MultiIconOnHide(self)
        if self.iconData then
            if #self.iconData > 1 then
    ...
    function FOB.MultiIconInitialise(self)
